...
predictTEST
```
Predicting out-of-Sample Error using the preferred model:RandomForest
```{r}
predictRandForest <- predict(modFitRandForest, newdata=Testset)
confMatRandForest <- confusionMatrix(predictRandForest, Testset$classe)
confMatRandForest
...
```{r}
sqrt(mean((predictRandForest - modFitRandForest$finalModel)^2))
sqrt(mean((predictTEST - predictRandForest)^2))
```